      ...
      <ThemeProvider theme={theme}>
        ...
        <main>
          <Container sx={{ py: 8 }} maxWidth="lg">
            <Grid container spacing={4} style={{paddingRight: 30, maxHeight: '90vh', overflow: 'auto'}}>
              {places.map((place, index) => (
                <Grid item key={place} xs={12} sm={6} md={4} lg={3}>
                  <Card>
                    <Link to={`/favoriteplaces/friends/${place.userId}`}>
                      <CardMedia
                        component="img"
                        height="250"
                        sx={{pt: "20%"}}
