...
function FriendsView(){
    ...
    const getFriendCard = (friend,index) => {
        return (
                <Grid key = {friend._id} item xs = {12} >
                    <Card sx = {{my: "10px", mx: "auto",width:"80%"}} onClick = {() => navigate(`/favoriteplaces/friends/${friend._id}`) }>
    ...
    return (
            <Box sx = {{mx:"auto",minHeight: "97vh", width: "50%", backgroundColor: "lightgrey"}} style={{maxHeight: '90vh', overflow: 'auto'}}>
                <Grid container >
                    ...
                    {friends.map((friend,index) => friend.name.toLowerCase().includes(filter) && (friend._id !== user._id) && getFriendCard(friend,index))}
                </Grid>
            </Box> 
    )
}
